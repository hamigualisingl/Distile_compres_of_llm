...
- 做法:压缩还原部分照旧,对齐阶段做法如下:样本分为俩个部分:(a,b）,俩次前向:第一次llm(a,b)取出b部分输出的概率分布,第二次:llm( 小模型(a) ,b),取出b部分输出的概率分布,计算俩者的kl散度.
- 还有一种离线做法:输入a,llm预测出序列b,得到数据对(a,b),然后训练llm( 小模型(a) ,b),loss为next_token预测损失.
